...
//Hàm mươn sách theo ID 
//trả về 1 nếu mượn thành công, 0 nếu không tìm thấy sách hoặc sách không có sẵn
int borrow_book(Book library[], int book_count, int id)
{
    int idx = find_book_by_id(library, book_count, id);
    if(idx < 0) 
    {
        printf("Cannot find the book with ID: %d\n", id);
        return 0;
    }
    if(library[idx].is_available == 1)
    {
        printf("The book with ID: %d is not available\n", id);
        return 0;
    }
    library[idx].is_available = 1; // Đánh dấu sách đã được mượn
    printf("The book with ID: %d was borrowed successfully\n", id);
    return 1;
}
...
int delete_book(Book library[], int *book_count, int id)
{
    ...
    printf("Index of book to delete: %d\n", idx);
    //if(idx < 0) return 0;   //không tìm thấy 
    if(idx < 0) 
    {
        printf("Cannot find the book with ID: %d\n", id);
    ...
    else
    {
        ...
        for(int i = idx; i < *book_count - idx; i++)
        {
